...
| deleteTimeoutLong | Same as above, but for messages that have a longer timeout.  | Only used in a few places. In milliseconds, default 6000. |
...
| level1, level2, level3 | Settable access levels that give users access to certain commands. | Default access levels can be found in [command reference](commands.md). |
...
- OAuth app
	1. We'll start off by creating an OAuth application in Discord's developer interface so that the bot can login.
		- Go to [https://discordapp.com/developers/applications/me](https://discordapp.com/developers/applications/me) and create a new application.
...
- API keys
	- General notice: Anything within these fields is sensitive and could do much harm if revealed to the public. **Therefore it is imperative that you keep this to yourself.** WildBeast maintainers will **NEVER** ask for your login details or API keys. Only show the options to trusted WildBeast maintainers with the `Staff` role in WildBot's Territory.
	- imgflip
    ...
    - Mashape
		...
		![Mashape App Page](screenshots/mashapeapp.png)
